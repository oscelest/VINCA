...
public class CreateDirectoryObjectRequest extends HttpRequest {
    public CreateDirectoryObjectRequest(String name, String parent_id, boolean folder, Response.Listener<JSONObject> responseListener) {
        super(Method.POST, DIRECTORY_URL, responseListener,
            new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Log.d("CreateDirectoryError", "There was an error.");
                }
            },
            new HttpParameter("name", name),
            new HttpParameter("parent_id", parent_id),
            new HttpParameter("folder", folder ? "true" : "false")
        );
    }
}
